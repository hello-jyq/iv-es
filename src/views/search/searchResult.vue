<template>
  <div class="result_box">
    <div :class="isCollapse?'result_DIY result_filter_all':'result_DIY result_filter'">
      <div class="title_box">
        <ul>
          <li v-for="(item,index) in filetab" :key="index" :class="activeDataSource==index?'active':''" :title="item.value" @click="filetapClick(item.id,index),activeDataSource=index">
            {{ item.value }}
          </li>
        </ul>
      </div>
      <div class="con_box">
        <div class="left_result">
          <div v-if="activeDataSource!='0'" class="search_num" @click="istree=!istree">
            <i class="iconfont icon-sousuo2" />
            <span>搜索结果数量</span>
            <i class="iconfont icon-ai-arrow-down" :class="istree?'':'icon_gif'" />
          </div>
          <el-tree
            ...
            @check="docTreeCheck"
          />
          <div v-if="activeDataSource!='0'" style="margin-top:30px" />
    ...
    <div class="container">
      <el-row class="search_box">
        <el-col :span="24">
          ...
          >
            ...
            <el-button slot="suffix" @click="btnSearch">
              搜&nbsp;&nbsp;索
            </el-button>
          </el-autocomplete>
        </el-col>
        <el-col :span="24" class="flex_ceter" style="height:24px;line-height:24px">
          ...
          <el-select v-model="docLanguage" popper-class="lang_select" placeholder="选择语言" placement="top-end" @change="getLang">
            <el-option
      ...
      <div v-loading="loading" class="main_box">
        ...
        <div v-if="results.results!=''" class="result_main">
          <div v-for="(item,index) in results.results" :key="item.id" class="each_reult">
            ...
            <div class="each_r_title">
              ...
              <span @click="showSLT(item)" v-html="item.fileName" />
            ...
            <div class="each_r_con">
              {{ item.updateTime }} {{ item.fileSize }}KB - {{ item.fileType }} <span v-html="item.fileContentOutline" />
            </div>
      ...
      <div v-show="results!=''" class="search_Pagination">
        <el-pagination
          :current-page="pageNo"
          background
        ...
        <el-pagination
          ...
          @current-change="handleCurrentChange"
        >
          <button type="button" class="last-pager" @click="jumpPage('0')">
        ...
        <el-pagination
          ...
          @current-change="handleCurrentChange"
        >
          <button type="button" class="first-pager" @click="jumpPage('1')">
    ...
    >
      <div class="each_r_top">
        <div>
          <span><i class="iconfont icon-wenjianjia" />
            {{ sltData.dataSource }}
          </span>
          <span><i class="iconfont icon-wendang" />
            {{ sltData.filePath }}
          </span>
          <span><i class="iconfont icon-wode" />
            {{ sltData.updateUser }}
          </span>
        </div>
      </div>
      ...
      <div class="slt_down" @click="down(sltData.fileUrl)">
    ...
    >
      ...
      <div class="diy_size_pop_button">
        ...
        <div class="submit_size" @click="submitSize">
          确&nbsp;定
        </div>
      </div>
    </el-dialog>
...
<script>
import { advancedSearch, getTerms, filterSearch } from '@/api/es/es-api'
